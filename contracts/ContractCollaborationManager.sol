...
contract ContractCollaborationManager {
    ...
    enum Operator{LESS,GREATER,EQUAL,NEQ,LEQ,GEQ}
    ...
    */
    function createTask(string memory _activity,
                        ...
                        Tasktype _tasktype,
                         uint[] memory _requirements) public {
    ...
    function getStringDecision(uint _id) public view returns(GatewayType gatewaytype,
        ...
        tasks[_id].decision.competitors, tasks[_id].decision.operator, tasks[_id].decision.stringoperants);           
    }
    ...
    function getIntegerDecision(uint _id) public view returns(GatewayType gatewaytype,
        ...
        require(testIfDecisionExists(_id),"task has no decision");
        return (tasks[_id].decision.gatewaytype, tasks[_id].decision.typ, tasks[_id].decision.completed,
        tasks[_id].decision.competitors, tasks[_id].decision.operator, tasks[_id].decision.integeroperants);               
    }
    ...
    function addIntDecisionToTaskId(uint _id,
                                     ...
                                     Operator _op,
                                      ...
                                      uint[] memory _competitors) public returns (bool) {
        ...
        if(testIfDecisionExists(_id) || testIfActionExists(_id)){
            ...
            tasks[_id].decision.competitors = _competitors;
    ...
    */
    function isDecisionClosed(uint _id) public view returns (bool) {
        ...
        for(uint i = 0; i < tasks[_id].decision.competitors.length; i++){
            uint comp_id = tasks[_id].decision.competitors[i];
...
library DecisionLibrary{
    ...
    function evaluate(uint _firstint, ContractCollaborationManager.Operator _op, uint _secondint)
