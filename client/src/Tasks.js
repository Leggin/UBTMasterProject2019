    ...
    static async getAllTasksWithDecisionPromise(contract, taskCallback) {
        ...
        taskIds.forEach(async taskId => {
            ...
            if (decisionExists) {
                result.operands = {};
                let decisionData;
                let decisionType = await contract.methods.getDecisionType(taskId).call();

                if (Number(decisionType) === 0) {
                    decisionData = await contract.methods.getStringDecision(taskId).call();

                    result.operands.global = decisionData.stringoperants[0];
                    result.operands.local = decisionData.stringoperants.local;
                } else {
                    decisionData = await contract.methods.getIntegerDecision(taskId).call();
                    result.operands.global = decisionData.integeroperants[0];
                    result.operands.local = decisionData.integeroperants.local;
                }

                result.decisionType = decisionType;
                result.competitors = decisionData.competitors;
